...
public class TestNGExtentReportListener implements ITestListener {
    ...
    public void onTestStart(ITestResult result) {
        ...
        if (className.contains("Login"))
            extentTest.assignCategory("Authentication");
        else if (className.contains("Cart"))
            extentTest.assignCategory("Cart Management");
        else if (className.contains("Payment"))
            extentTest.assignCategory("Payment");
        else if (className.contains("Order"))
            extentTest.assignCategory("Order Processing");
        else if (className.contains("Report"))
            extentTest.assignCategory("Reports");
        else
            extentTest.assignCategory("General API");
    ...
    /**
     * Injects context-specific business data into the report based on the test
     * method execution.
     */
    private void injectBusinessData(ITestResult result) {
        ...
        if (methodName.contains("Login") || methodName.contains("Setup")) {
            test.get().info("<span class='badge badge-" + badgeColor + "' style='font-size: 1.2em'>" + userTypeLabel
                    + "</span>");
        }
        ...
        try {
            ...
            if (methodName.contains("Login") || methodName.contains("Setup")) {
                ...
                if (userId == null && "member".equalsIgnoreCase(userType))
                    userId = RequestContext.getMemberUserId();
                if (userId == null && "non_member".equalsIgnoreCase(userType))
                    userId = RequestContext.getNonMemberUserId();
                if (userId == null && "new_user".equalsIgnoreCase(userType))
                    userId = RequestContext.getNewUserUserId();

                if (token == null && "member".equalsIgnoreCase(userType))
                    token = RequestContext.getMemberToken();
                if (token == null && "non_member".equalsIgnoreCase(userType))
                    token = RequestContext.getNonMemberToken();
                if (token == null && "new_user".equalsIgnoreCase(userType))
                    token = RequestContext.getNewUserToken();

                test.get().info("<b>User Inputs & Generated Data (" + userTypeLabel + "):</b>");
                if (mobile != null)
                    test.get().info("   - Input Mobile: " + mobile);
                test.get().info("   - Generated User ID: " + (userId != null ? userId : "N/A"));
                test.get()
                        .info("   - Token Status: " + (token != null && !token.isEmpty()
            ...
            // SEARCH & ADD TO CART
                ...
                if (cartItems != null && !cartItems.isEmpty()) {
                    ...
                    if (itemsObj.length() > 2)
                        itemsObj.setLength(itemsObj.length() - 2);
                ...
                // Fallback to Global Search Selection
                    ...
                    if (tests != null && !tests.isEmpty()) {
                        ...
                        if (testsAdded.length() > 2)
                            testsAdded.setLength(testsAdded.length() - 2);
                ...
                test.get().info("   - Cart ID Generated: "
                        + (RequestContext.getCurrentCartId() != null ? RequestContext.getCurrentCartId() : "N/A"));
            ...
            // SLOT & ADDRESS
                ...
                test.get()
                        .info("   - Address GUID: " + (RequestContext.getCurrentAddressGuid() != null
                                ? RequestContext.getCurrentAddressGuid()
                                : "N/A"));
            }
            ...
            // ORDER & ASSIGNMENT
                ...
                if (orderId == null && "member".equalsIgnoreCase(userType))
                    orderId = RequestContext.getMemberOrderId();
                if (orderId == null && "non_member".equalsIgnoreCase(userType))
                    orderId = RequestContext.getNonMemberOrderId();
                if (orderId == null && "new_user".equalsIgnoreCase(userType))
            ...
            // REPORT GENERATION
                ...
                test.get().info(
                        "   - Validation Status: <span class='badge badge-success'>100% Data Fidelity Match</span>");
            }
        } catch (Exception e) {
